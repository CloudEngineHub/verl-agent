...
class NaiveChatCompletionScheduler(ChatCompletionScheduler):
    ...
    async def generate_sequences(self, batch: DataProto, **sampling_params) -> DataProto:
        ...
        return self._postprocess(batch, batch_conversations, kwargs["n"], self.config.prompt_length, self.config.response_length)

    def _postprocess(self, batch: DataProto, batch_conversations: List[List[List[Dict[str, str]]]], n: int, max_prompt_length: int, max_response_length: int) -> DataProto:
        ...
        prompts = self.tokenizer(prompts, return_tensors="pt", padding="max_length", max_length=max_prompt_length, padding_side="left")
        responses = self.tokenizer(responses, return_tensors="pt", padding="max_length", max_length=max_response_length, truncation=True, padding_side="right")
        if n > 1:
